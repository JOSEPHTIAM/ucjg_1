...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
     <head>
          ...
          <title>Page de modification de la formation pdf du coté administrateur</title>
          ...
          <style>
               body {
                    background: url('{{ asset('image/Final_formation') }}') no-repeat center center;
     ...
     <body>
          ...
          <div class="container">
            ...
            <div class="form-container">
                ...
                <div class="form-left">  
                    <h2 align="center">
                        <font color="#000076"><b><strong>Modification de la formation pdf</strong></b></font>
                    </h2>
                    @if(session('error'))
                        ...
                        <div class="form-group">
                            ...
                            <select name="id_localisation" id="id_localisation" class="form-control" required>
                                @foreach(\App\Models\Localisation::all() as $localisation)
                                    <option value="{{ $localisation->id_localisation }}" {{ $formation1->id_localisation == $localisation->id_localisation ? 'selected' : '' }}>
                        ...
                        <div class="form-group">
                            ...
                            <select name="id_magasin" id="id_magasin" class="form-control" required onchange="calculateTotal()">
                                @foreach(\App\Models\Magasin::all() as $magasin)
                                    <option value="{{ $magasin->id_magasin }}" {{ $formation1->id_magasin == $magasin->id_magasin ? 'selected' : '' }}>
                        ...
                        <div class="form-group">
                            ...
                            <select name="matricule" id="matricule" class="form-control" required>
                                @foreach(\App\Models\User::where('role', 'Administrateur')->orWhere('role', 'Agent')->get() as $user)
                                    <option value="{{ $user->matricule }}" {{ $formation1->matricule == $user->matricule ? 'selected' : '' }}>
